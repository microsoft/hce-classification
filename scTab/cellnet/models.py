...
def augment_data(x: torch.Tensor, augmentation_vectors: torch.Tensor):
    ...
    sign = 2.0 * (torch.bernoulli(0.5 * torch.ones(x.shape[0], 1, device=x.device)) - 0.5)

    return torch.clamp(x + (sign * augmentations), min=0.0, max=9.0)
...
class BaseClassifier(pl.LightningModule, abc.ABC):
    ...
    def __init__(
        ...
        print(f"Using hierarchical loss: {self.use_hierarchical_loss}")
        ...
        self.register_buffer(
            "child_lookup_transposed",
            torch.transpose(torch.tensor(child_matrix.astype("f4")), 0, 1),
        )
    ...
    def _sctab_loss(self, logits, targets, weight=None):
        return F.cross_entropy(logits, targets, weight=weight)

    def _loss(self, logits, targets, weight=None):
        if self.use_hierarchical_loss:
            return self._hierarchical_loss(logits, targets, weight=weight)
        return self._sctab_loss(logits, targets, weight=weight)
...
class LinearClassifier(BaseClassifier):
    def __init__(
        ...
        self.save_hyperparameters(ignore=["class_weights", "child_matrix"])

        self.classifier = nn.Linear(gene_dim, type_dim)

    def _step(self, batch, training=True):
        x = batch["X"]  # Should we augment the data here?
        targets = batch["cell_type"]
        logits = self(x)
        with torch.no_grad():
            preds = torch.argmax(logits, dim=1)
            preds_corrected, targets_corrected = self.hierarchy_correct(preds, targets)
        if training:
            loss = self._loss(logits, targets, weight=self.class_weights)
        else:
            loss = self._loss(logits, targets_corrected)
...
class TabnetClassifier(BaseClassifier):
    def __init__(
        ...
        self.save_hyperparameters(
            ignore=["class_weights", "child_matrix", "augmentations"]
        )
    ...
    def _step(self, batch, training=True):
        x = (
            augment_data(batch["X"], self.augmentations)
            if (self.augment_training_data and training)
            else batch["X"]
        )
        targets = batch["cell_type"]
        ...
        if training:
            loss = (
                self._loss(logits, targets, weight=self.class_weights)
                - self.lambda_sparse * m_loss
            )
        else:
            loss = self._loss(logits, targets_corrected) - self.lambda_sparse * m_loss
