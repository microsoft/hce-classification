...
def set_default_kwargs_dataset(kwargs: Dict[str, any] = None, training: bool = True):
    ...
    if all(["part_size" not in kwargs, "part_mem_fraction" not in kwargs]):
        kwargs["part_size"] = "128MB"  # if training else '325MB'
...
def _get_data_files(base_path: str, split: str, sub_sample_frac: float):
    ...
    files = [
        join(base_path, split, file) for file in sorted(files, key=lambda x: int(x.split(".")[1]))
    ]
...
class MerlinDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.dataloader_kwargs_train = set_default_kwargs_dataloader(
            dataloader_kwargs_train, training=True
        )
        self.dataloader_kwargs_inference = set_default_kwargs_dataloader(
            dataloader_kwargs_inference, training=False
        )
    ...
    def train_dataloader(self):
        return Loader(
            self.train_dataset, batch_size=self.batch_size, **self.dataloader_kwargs_train
        )

    def val_dataloader(self):
        return Loader(
            self.val_dataset, batch_size=self.batch_size, **self.dataloader_kwargs_inference
        )

    def test_dataloader(self):
        return Loader(
            self.test_dataset, batch_size=self.batch_size, **self.dataloader_kwargs_inference
        )
