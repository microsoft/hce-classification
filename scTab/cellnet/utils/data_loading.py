...
def streamline_count_matrix(x_raw, gene_names_raw, gene_names_model):
    ...
    return csr_matrix(
        (data, (row, col)), shape=(x_raw.shape[0], len(gene_names_model)), dtype="f4"
    )
...
def dataloader_factory(x, obs=None, batch_size=2048, shuffle=False):
    if shuffle:
        sampler = BatchSampler(
            RandomSampler(range(x.shape[0])), batch_size=batch_size, drop_last=True
        )
    else:
        sampler = BatchSampler(
            SequentialSampler(range(x.shape[0])), batch_size=batch_size, drop_last=False
        )
